package com.benglasser.http.request.handlers;

import com.benglasser.http.header.Header;
...
public class RequestHandler implements Runnable {
  ...
  /**
   * parse request for connection and returns a Header object.  When populating
   * the Header object the parser converts all header fields to lower case to
   ...
   */
  @SneakyThrows
  private Header parseRequest() {
    log.info("parsing request: {}", connection);
    Header.RequestBuilder requestBuilder = Header.builder();
    String inputLine = ".:.";
    ...
    try {
      ...
      while (inputLine != null && !inputLine.equals("")) {
        ...
        if (Pattern.compile("(GET|POST|PUT|PATCH|DELETE)", Pattern.CASE_INSENSITIVE).matcher(inputLine).find()) {
          //  Header-Line   = Method SP Header-URI SP HTTP-Version CRLF
  ...
  private void parseRequestLine(Header.RequestBuilder builder, String requestLine)
  {
    //  Header-Line   = Method SP Header-URI SP HTTP-Version CRLF
  ...
  @Override
  public void run() {
    Header header = this.parseRequest();
    if (header.getMethod().contains("get")) {
      ...
      try (
        ...
        String path = rootDirectory + header.getRequestUri();
      ...
      catch (NoSuchFileException e)
      {
        ...
        log.error("Requested file not found: {}", header.getRequestUri());
      }
      catch (IOException e)
      {
