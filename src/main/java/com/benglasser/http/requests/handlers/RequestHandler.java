...
public class RequestHandler implements Runnable
{
  ...
  /**
   * parse request for connection and returns a Request object. When populating the Request object
   * the parser converts all header fields to lower case to make them easier to work with later on.
   *
   ...
   */
  @SneakyThrows
  private Optional<Request> parseRequest()
  {
    ...
    try
    {
      ...
      while (inputLine != null && !inputLine.equals(""))
      {
        ...
        if (Pattern.compile("(GET|POST|PUT|PATCH|DELETE)", Pattern.CASE_INSENSITIVE)
            .matcher(inputLine).find())
        {
      ...
      return Optional.ofNullable(requestBuilder.build());
    }
    catch (Exception e)
    {
  ...
  public void run()
  {
    final Optional<Request> request = this.parseRequest();
    request
        .map((x) ->
        {
          if (x.getMethod().toLowerCase().contains("get"))
          {

            // try with resources is useful here to make sure we close the connection once we have
            // responded
        try (
            PrintWriter out =
                new PrintWriter(connection.getOutputStream(), true))
        {
          String path = rootDirectory + x.getRequestUri();
        ...
        catch (NoSuchFileException e)
        {
          ...
          log.error("Requested file not found: {}", x.getRequestUri());
        }
        catch (IOException e)
        {
        ...
        {
          log.error("Unexpected exception: {}, Request; {}", e.getMessage(), request);
        }
